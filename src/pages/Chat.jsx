...
const Chat = () => {
  ...
  const [showLoanDropdown, setShowLoanDropdown] = useState(false); // 도서 이용 안내 드롭다운
  ...
  const handleSuggestedClick = (type) => {
    ...
    setShowLoanDropdown(false); // 드롭다운 닫기
    setMessages((prev) => [
      ...prev,
  ...
  const callApiAndRender = async (question) => {
    ...
    setMessages((prev) => {
      const next = prev.filter(
        (m) => !(m.role === "assistant" && m.type === "typing")
      );
      next.push({
        role: "assistant",
        type: "typing",
      ...
      if (!(major === "컴퓨터공학부" && allowedYears.includes(year))) {
        setMessages((prev) => [
          ...prev,
          {
            ...
            type: "error",
            content:
              "해당 데이터는 2학년 이상 컴퓨터공학과 학생의 정보만 포함합니다. 조건을 확인해 주세요.",
            timestamp: time,
          },
        ]);
        return;
      ...
      <main className="chat-main">
        <ul className="message-list">
          {messages.map((msg, idx) => {
            if (msg.type === "typing")
              return <TypingBubble key={`typing-${idx}`} />;
        ...
        {showSuggestions && !recommendFlow && (
          <div className="suggested-questions">
            ...
            <div className="button-group">
              ...
              <button onClick={() => handleSuggestedClick("hours")}>
                ⏰ 오픈 / 마감
              </button>

              {/* 드롭다운 버튼 */}
              <div className="loan-dropdown-container">
                <button
                  onClick={() => handleSuggestedClick("loan")}
                  className={`loan-dropdown-button ${
                    showLoanDropdown ? "active" : ""
                  }`}
                >
                  📦 도서 이용 안내 {showLoanDropdown}
                </button>
                {showLoanDropdown && (
                  <div className="loan-dropdown-menu">
                    <button onClick={() => handleSuggestedClick("return")}>
                      도서 반납/연체 안내
                    </button>
                    <button onClick={() => handleSuggestedClick("extend")}>
                      도서 연장/예약/분실
                    </button>
                    <button onClick={() => handleSuggestedClick("precautions")}>
                      도서관 이용 시 주의사항
                    </button>
                    <button onClick={() => handleSuggestedClick("graduate")}>
                      도서관 졸업생 이용안내
                    </button>
                    <button onClick={() => handleSuggestedClick("member")}>
                      도서관 성북/종로구민 이용안내
                    </button>
                    <button onClick={() => handleSuggestedClick("disability")}>
                      도서관 장애 학생지원제도
                    </button>
                  </div>
                )}
              </div>
              <button onClick={() => handleSuggestedClick("homepage")}>
                🌐 홈페이지
              </button>
            </div>
          </div>
